...
# ------------------------------------------------------------
# 7️⃣ 서버 실행 (로컬/배포 환경 호환)
# ------------------------------------------------------------
if __name__ == "__main__":
    port = int(os.getenv("PORT", 5000))
    host = os.getenv("HOST", "0.0.0.0")
    
    # Render 배포 환경에서는 gunicorn 사용, 로컬에서는 gevent 사용
    if os.getenv("RENDER"):
        # Render 환경에서는 gunicorn이 실행하므로 여기서는 실행하지 않음
        pass
    else:
        # 로컬 개발 환경: Gevent 사용
        ...
        print(f"✅ Gevent WSGIServer running on http://{host}:{port}")
