...
function App() {
  ...
  const sendToBackend = useCallback(async (endpoint, data = {}) => {
    try {
      ...
      if (err.message.includes('Failed to fetch') || err.message.includes('NetworkError') || err.message.includes('가져오지 못했습니다')) {
        throw new Error(`백엔드 서버에 연결할 수 없습니다. 백엔드 서버가 실행 중인지 확인해주세요. (${API_BASE_URL})`);
      }
      throw err;
    }
  }, [input, highlightMode]);
  ...
  const handleStream = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg = err.message.includes('Failed to fetch') || err.message.includes('NetworkError')
        ? `백엔드 서버에 연결할 수 없습니다. 백엔드 서버가 실행 중인지 확인해주세요. (${API_BASE_URL})`
        : err.message;
  ...
  const handleCorrectSpacing = useCallback(async () => {
    ...
    setSpacingText("");
    try {
      const response = await fetch(`${API_BASE_URL}/correct-spacing`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          message: input,
